...
class TopicController extends CommonController {
	...
	public function index(){
		...
		$table = M('topic'); // 实例化User对象
		
		//接收查询数据
		
		if (I('get.keyword')){
			$keyword = I('get.keyword');
			$data['t_Topic.title'] = array('like',"%{$keyword}%");
		}
		if (I('get.type')){
		...
		if(I('get.type')==2){
			$res = $table
			->join("left join t_user on t_topic.uid=t_user.id")
			->field("t_topic.*,t_user.username")
	...
	public function add(){
		if (I('post.')){
			...
			if ($res){
				...
				alertLocation('发布成功！', '/Admin/Topic/index/userid/'.$where['uid']);
			}else {
				$this->error('发布失败！');
			}
		}
	...
	public function edit(){
		...
		if (IS_POST){
			...
			$table->save($where);
 			if ($data['user_simg']){
				$img = M('img');
				$img->where("pid = '{$where['id']}' and type='Topic'")->delete();
	...
	//	$selfcity = M('selfcity');
		$img = M('img');
		$simg = $img->where("pid = $id and type='Topic'")->order('id asc')->select();
	...
	public function state(){
		...
		$table = M('Topic');
		if ($table->save($data)){	
	$this->redirect("/Admin/Topic/index".$str);
		}else {
			$this->error('没有任何修改！');
		}
	}
